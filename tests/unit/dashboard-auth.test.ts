...
describe('dashboard route auth middleware', () => {
  it('rejects request without auth headers', async () => {
    const env = createEnv();

    const response = await dashboardRoutes.fetch(
  ...
  it('enforces admin IP allowlist when configured', async () => {
    ...
    const denied = await dashboardRoutes.fetch(
      new Request('http://localhost/unknown', {
        headers: {
          authorization: 'Bearer admin-token',
    ...
    const allowed = await dashboardRoutes.fetch(
      new Request('http://localhost/unknown', {
        headers: {
          authorization: 'Bearer admin-token',
